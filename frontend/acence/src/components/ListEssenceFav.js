...
const ListEssence = (props) => {
    ...
    function removeToFavoris(id) {
        if (id !== undefined) {
            ...
            setFavoriteStationList(storageStation);
            Utils.default.sendRequest('POST', '/user/removeFavorite', JSON.stringify({ 'idStation': id, 'user': localStorage.getItem("token") }), () => {}).then(r => localStorage.setItem("favoriteStations", JSON.stringify(storageStation)))
        } else {
